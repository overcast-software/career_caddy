...
def main():
    ...
    try:
        ...
        if os.getenv("USERNAME"):
            ...
            if user_count == 1:
                ...
                raise ValueError(
                    ...
                    "Please create a user first using load_user.py"
                )
            else:
                raise ValueError(
                    "Multiple users found."
                    "Please set the USERNAME environment variable"
                    "to specify which user to use."
                )
        args = parse_arguments()
        resume_path = args.resume or os.getenv("RESUME_PATH")
        if not resume_path:
            raise ValueError(
                "A resume file path must be provided either as an argument"
                "or through the RESUME_PATH environment variable."
            )
        ...
        match_score = scorer.score_job_match(
            job_handler.job.description, resume_content
        )
