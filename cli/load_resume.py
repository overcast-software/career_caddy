...
def main():
    ...
    if args.user_email:
        ...
        if not user:
            raise ValueError(f"No user found with email {args.user_email}. Please create one with cli/load_user.py")
        ...
        if user_count == 1:
            ...
            raise ValueError("Multiple users found. Please set USERNAME env var or use --user-email to select a user.")
    ...
    if not resume_path:
        raise ValueError("Provide a resume file with --resume or set RESUME_PATH; or use --dir to import a directory.")
